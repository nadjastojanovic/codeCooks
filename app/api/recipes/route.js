...
export async function GET(request) {
    try {
        ...
        if (!tag || tag === "All") { // default
            // ARRAY_AGG makes sure no duplicate children shown in All if a recipe has more than one tag
            qs = `
            ...
            GROUP BY r.id, r.title, r.image_url, f.user_id;`;
            values = [userId];
        } else { // selected tag
            qs = `
